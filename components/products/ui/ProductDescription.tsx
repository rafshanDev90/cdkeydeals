  ...
  return (
    <section className="mb-16">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <h2 className="text-3xl font-bold text-zinc-50 font-inter tracking-tight">
          Product Description
        </h2>
      </div>
      ...
      <div className="space-y-8">
        ...
        {product.description ? (
          <div className="bg-white/5 backdrop-blur-sm border border-zinc-800/50 rounded-2xl p-8 lg:p-10">
            <div 
              className="text-zinc-300 text-lg leading-relaxed font-inter text-justify space-y-6"
              dangerouslySetInnerHTML={{ __html: product.description }}
            />
          </div>
        ) : (
          <div className="bg-white/5 backdrop-blur-sm border border-zinc-800/50 rounded-2xl p-8 lg:p-10">
            <p className="text-zinc-400 text-lg leading-relaxed font-inter text-justify italic">
        ...
        {product.features && product.features.length > 0 && (
          <div className="bg-gradient-to-br from-zinc-900/50 to-zinc-800/30 backdrop-blur-md border border-zinc-700/50 rounded-3xl p-8 lg:p-10 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h3 className="text-2xl font-bold mb-6 text-zinc-50 font-inter flex items-center gap-3">
              ...
              <div className="grid gap-4">
                {product.features.map((feature, index) => (
                  <div 
                    key={index}
                    className="flex items-start gap-4 p-4 rounded-xl bg-white/5 border border-zinc-700/30 hover:bg-white/10 transition-all duration-300 group"
                  >
                    ...
                    <p className="text-zinc-200 text-base font-inter leading-relaxed flex-1">
                      {feature}
                    </p>
                  </div>
                ))}
        ...
        {systemRequirements.length > 0 && (
          <div className="bg-white/5 backdrop-blur-sm border border-zinc-800/50 rounded-2xl overflow-hidden">
            <div className="bg-gradient-to-r from-zinc-800/50 to-zinc-700/30 px-8 py-6 border-b border-zinc-700/50">
              <h3 className="text-2xl font-bold text-zinc-50 font-inter flex items-center gap-3">
                ...
                <table className="w-full">
                  <tbody>
                    {systemRequirements.map((req, index) => (
                      <tr 
                        key={index}
                        className="border-b border-zinc-700/30 last:border-b-0 hover:bg-white/5 transition-colors"
                      >
                        <td className="py-4 pr-8 w-1/3">
                          <div className="flex items-center gap-3">
                            {req.icon && (
                              <div className="w-8 h-8 bg-zinc-700/30 rounded-lg flex items-center justify-center flex-shrink-0">
                                {req.icon}
                              </div>
                            )}
                            <span className="font-semibold text-zinc-200 font-inter">
                              {req.label}
                            </span>
                          </div>
                        </td>
                        <td className="py-4 pl-8 w-2/3">
                          <span className="text-zinc-400 font-inter">
                            {req.value}
                          </span>
                        </td>
                      </tr>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold text-zinc-200 font-inter mb-1">
                          {req.label}
                        </p>
