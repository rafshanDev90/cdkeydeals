  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-baseline gap-3 flex-wrap">
        <span className="text-3xl lg:text-4xl font-bold text-gray-900 dark:text-zinc-50">{currentPrice}</span>
        {hasDiscount && originalPriceFormatted && (
          <>
            <span className="text-xl text-gray-500 dark:text-zinc-500 line-through">{originalPriceFormatted}</span>
            <span className="text-sm font-bold text-red-600 bg-red-50 dark:bg-red-950/50 border border-red-200 dark:border-red-800/50 px-2 py-1 rounded">
      ...
      <div className="flex items-center gap-4">
        <span className="font-medium text-gray-700 dark:text-zinc-50">Quantity:</span>
        <div className="flex items-center border border-gray-300 dark:border-zinc-700 rounded-lg overflow-hidden bg-white dark:bg-[#1a1a1a]">
          <button
            onClick={() => setQuantity((prev) => Math.max(1, prev - 1))}
            className="p-3 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors disabled:opacity-40 text-gray-700 dark:text-zinc-50"
            disabled={quantity <= 1}
            aria-label="Decrease quantity"
          >
            <Minus className="w-4 h-4" />
          </button>
          <span className="px-6 py-3 font-medium min-w-[4rem] text-center text-gray-900 dark:text-zinc-50">{quantity}</span>
          <button
      ...
      {/* Total Price */}
      <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-[#1a1a1a] border border-gray-200 dark:border-zinc-700 rounded-xl">
        <span className="font-medium text-gray-900 dark:text-zinc-50">Total:</span>
