    ...
    <MegaMenuWrapper isOpen={isOpen} onClose={onClose}>
      <div className="rounded-xl overflow-hidden">
        <div className="flex h-[450px]">
          ...
          <div className="w-1/3 bg-slate-50 p-6 border-r border-gray-200">
                  ...
                  <div 
                    className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all duration-200 ${
                      activeCategory === category.id 
                        ...
                        : 'hover:bg-white hover:text-purple-600 text-slate-700'
                    }`}
                  >
                    <category.icon className={`w-5 h-5 transition-colors duration-200 ${
                      activeCategory === category.id ? 'text-purple-600' : 'text-slate-500 group-hover:text-purple-600'
                    }`} />
                    <span className={`font-medium transition-colors duration-200 text-sm ${
                      activeCategory === category.id ? 'text-purple-600' : 'text-slate-700 group-hover:text-purple-600'
                    }`}>
                      {category.title}
                    </span>
                    <ChevronRight className={`w-4 h-4 ml-auto transition-colors duration-200 ${
                      activeCategory === category.id ? 'text-purple-600' : 'text-slate-400 group-hover:text-purple-600'
                    }`} />
                  </div>
                </div>
              ))}
            ...
            {getActiveCategoryData() ? (
                  ...
                  {getActiveCategoryData()?.submenu.map((submenuItem, index) => {
                    ...
                    return (
                      <a
                        key={submenuItem}
                        href="#"
                        className="flex items-center px-4 py-3 text-slate-700 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition-all duration-200 group"
                        onClick={onClose}
                      >
                        <ItemIcon className="w-4 h-4 mr-3 text-slate-400 group-hover:text-purple-600 transition-colors duration-200" />
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  ...
                  <p className="text-slate-500 text-sm font-medium mb-2">Explore Games by Category</p>
                  <p className="text-slate-400 text-xs">Browse by Platform, Category, Region, or Extras</p>
                </div>
              </div>
            )}
          </div>
